...
def word_cloud(inputFileName, del_word):
    ...
    for item in data:
        if 'description' in item.keys():
            # description 내의 문자나 숫자를 제외한 나머지를 공백으로 치환한후
            # 모든 기사의 description을 연결하여 하나의 문장(post)으로 출력
            post = post + re.sub(r'[^\w]',' ',item['description'])+''

    # print(post)

    nlp = Okt() # Okt 클래스로 객체 nlp를 생성
    post_N = nlp.nouns(post) # post에서 명사품사 추출
    # print(post_N)

    count = Counter(post_N) # 단어의 빈도수를 dict형태로 출력
    # print(count)
    ...
    word_value = word_count.values() # word_count에서 value값만 추출
    ...
    w_cloud= wc.generate_from_frequencies(word_count)
    plt.imshow(w_cloud)
    plt.axis('off')
    plt.show()